// loadsysDlg.cpp : implementation file
//

#include "stdafx.h"
#include "loadsys.h"
#include "loadsysDlg.h"
#include "winsvc.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif
BOOL LoadNTDriver(char* lpDriverName,char* lpDriverPathName);
BOOL UnLoadNTDriver(char* lpDriverName);

/////////////////////////////////////////////////////////////////////////////
// CAboutDlg dialog used for App About

class CAboutDlg : public CDialog
{
public:
	CAboutDlg();

// Dialog Data
	//{{AFX_DATA(CAboutDlg)
	enum { IDD = IDD_ABOUTBOX };
	//}}AFX_DATA

	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CAboutDlg)
	protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL

// Implementation
protected:
	//{{AFX_MSG(CAboutDlg)
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};

CAboutDlg::CAboutDlg() : CDialog(CAboutDlg::IDD)
{
	//{{AFX_DATA_INIT(CAboutDlg)
	//}}AFX_DATA_INIT
}

void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CAboutDlg)
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CAboutDlg, CDialog)
	//{{AFX_MSG_MAP(CAboutDlg)
		// No message handlers
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CLoadsysDlg dialog

CLoadsysDlg::CLoadsysDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CLoadsysDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CLoadsysDlg)
	m_syspathname = _T("");
	//}}AFX_DATA_INIT
	// Note that LoadIcon does not require a subsequent DestroyIcon in Win32
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CLoadsysDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CLoadsysDlg)
	DDX_Text(pDX, IDC_EDIT_SYSPATHNAME, m_syspathname);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CLoadsysDlg, CDialog)
	//{{AFX_MSG_MAP(CLoadsysDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_LOADSYS, OnButtonLoadsys)
	ON_BN_CLICKED(IDC_BUTTON_UNLOADSYS, OnButtonUnloadsys)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CLoadsysDlg message handlers

BOOL CLoadsysDlg::OnInitDialog()
{
	CDialog::OnInitDialog();

	// Add "About..." menu item to system menu.

	// IDM_ABOUTBOX must be in the system command range.
	ASSERT((IDM_ABOUTBOX & 0xFFF0) == IDM_ABOUTBOX);
	ASSERT(IDM_ABOUTBOX < 0xF000);

	CMenu* pSysMenu = GetSystemMenu(FALSE);
	if (pSysMenu != NULL)
	{
		CString strAboutMenu;
		strAboutMenu.LoadString(IDS_ABOUTBOX);
		if (!strAboutMenu.IsEmpty())
		{
			pSysMenu->AppendMenu(MF_SEPARATOR);
			pSysMenu->AppendMenu(MF_STRING, IDM_ABOUTBOX, strAboutMenu);
		}
	}

	// Set the icon for this dialog.  The framework does this automatically
	//  when the application's main window is not a dialog
	SetIcon(m_hIcon, TRUE);			// Set big icon
	SetIcon(m_hIcon, FALSE);		// Set small icon
	
	// TODO: Add extra initialization here
	
	return TRUE;  // return TRUE  unless you set the focus to a control
}

void CLoadsysDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	if ((nID & 0xFFF0) == IDM_ABOUTBOX)
	{
		CAboutDlg dlgAbout;
		dlgAbout.DoModal();
	}
	else
	{
		CDialog::OnSysCommand(nID, lParam);
	}
}

// If you add a minimize button to your dialog, you will need the code below
//  to draw the icon.  For MFC applications using the document/view model,
//  this is automatically done for you by the framework.

void CLoadsysDlg::OnPaint() 
{
	if (IsIconic())
	{
		CPaintDC dc(this); // device context for painting

		SendMessage(WM_ICONERASEBKGND, (WPARAM) dc.GetSafeHdc(), 0);

		// Center icon in client rectangle
		int cxIcon = GetSystemMetrics(SM_CXICON);
		int cyIcon = GetSystemMetrics(SM_CYICON);
		CRect rect;
		GetClientRect(&rect);
		int x = (rect.Width() - cxIcon + 1) / 2;
		int y = (rect.Height() - cyIcon + 1) / 2;

		// Draw the icon
		dc.DrawIcon(x, y, m_hIcon);
	}
	else
	{
		CDialog::OnPaint();
	}
}

// The system calls this to obtain the cursor to display while the user drags
//  the minimized window.
HCURSOR CLoadsysDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

CString DriverName,PathName;
void CLoadsysDlg::OnButtonLoadsys() 
{
	CFileDialog sysfile(true,NULL,NULL,0,"驱动文件(*.sys)|*.sys|所有文件(*.*)|*.*|");
	if(IDOK == sysfile.DoModal() )
	{
		m_syspathname=sysfile.GetPathName();
		UpdateData(false);//更新到窗口
		
		DriverName=sysfile.GetFileName();
		PathName=sysfile.GetPathName();
		if(LoadNTDriver(DriverName.GetBuffer(256),PathName.GetBuffer(256))==true)
			AfxMessageBox("驱动启动成功");
	}


}

BOOL LoadNTDriver(char* lpDriverName,char* lpDriverPathName)
{
//	A.OpenSCManager
//  B.CreateService
//  C.OpenService
//  D.StartService
//  E.CloseServiceHandle
	SC_HANDLE hSCM=NULL;
	SC_HANDLE hService=NULL;
	BOOL ret=false;
	
	hSCM=OpenSCManager(NULL,NULL,SC_MANAGER_ALL_ACCESS);
	if (!hSCM)
	{
		AfxMessageBox("OpenSCManager失败!");
		goto exits;
	}
	
	 hService=CreateService(
		  hSCM,       // handle to SCM database 
		  lpDriverName,      // name of service to start
		  lpDriverName,      // display name
		  SERVICE_ALL_ACCESS,      // type of access to service
		  SERVICE_KERNEL_DRIVER,        // type of service
		  SERVICE_AUTO_START,          // when to start service
		  SERVICE_ERROR_IGNORE,       // severity of service failure
		  lpDriverPathName,   // name of binary file
		  NULL,   // name of load ordering group
		  NULL,          // tag identifier
		  NULL,     // array of dependency names
		  NULL, // account name 
		  NULL          // account password
	);
	
	DWORD dwError;//放错误信息
	if (!hService)
	{
		dwError=GetLastError();
		if( dwError != ERROR_IO_PENDING && dwError != ERROR_SERVICE_EXISTS )  
		{
			AfxMessageBox("CreateService失败!其他原因");
			goto exits;
		}
		else
		{
			AfxMessageBox("CreateService服务失败!已经创建过!");
		}
	}
	else
	{
		AfxMessageBox("驱动服务创建成功");
	}
	
	hService=OpenService(hSCM,lpDriverName,SERVICE_ALL_ACCESS);
	if (!hService)
	{
		AfxMessageBox("OpenService失败!");
		goto exits;
	}

	ret=StartService(hService,NULL,NULL);
	if (!ret)
	{
		AfxMessageBox("StartService失败,可能服务已经开启!");
		goto exits;
	}
	ret=true;
exits:
	if (hService)
	{
		CloseServiceHandle(hService);
	}
	if (hSCM)
	{
		CloseServiceHandle(hSCM);
	}
	
	return ret;
}

BOOL UnLoadNTDriver(char* lpDriverName)
{
//	A.OpenSCManager
//	B.OpenService
//	C.ControlService
//	D.DeleteService
	SC_HANDLE hSCM=NULL;
	SC_HANDLE hService=NULL;
	SERVICE_STATUS SvrSta;
	BOOL ret=false;
	
	hSCM=OpenSCManager(NULL,NULL,SC_MANAGER_ALL_ACCESS);
	if (!hSCM)
	{
		AfxMessageBox("OpenSCManager失败!");
		goto exits;
	}
	
	hService=OpenService(hSCM,lpDriverName,SERVICE_ALL_ACCESS);
	if (!hService)
	{
		AfxMessageBox("OpenService失败!");
		goto  exits;
	}
	
	if (!ControlService(hService,SERVICE_CONTROL_STOP,&SvrSta))
	{
		AfxMessageBox("ControlService驱动停止失败!");
	}

	if (!DeleteService(hService))
	{
		AfxMessageBox("驱动卸载失败!");
	}
	else
	{
		ret=true;
	}
exits:
	if (hSCM)
	{
		CloseServiceHandle(hSCM);
	}
	if (hService)
	{
		CloseServiceHandle(hService);
	}
	return ret;
}

void CLoadsysDlg::OnButtonUnloadsys() 
{
	if(UnLoadNTDriver(DriverName.GetBuffer(256))==true);
	    AfxMessageBox("驱动卸载成功");
}

